 ...
 * {@link GameCameraFocusable} if contained in a "room" (otherwise camera does nothing). Game rooms are stored within a map as
...
public class RoomsGameCameraManager extends GameCameraManager {
    ...
    private final GameCameraFocusable gameCameraFocusable;
     ...
     * @param gameCameraFocusable          the gameCameraFocusable
     * @param gameRooms          the game rooms
     * @throws RoomsGameCameraHandlerException thrown if camera or gameCameraFocusable is null or if game rooms is empty or if
     *                                         transition duration is less than or equal to 0
     */
    public RoomsGameCameraManager(OrthographicCamera camera, GameCameraFocusable gameCameraFocusable,
                                  Map<Rectangle, String> gameRooms, Timer transitionTimer)
            throws RoomsGameCameraHandlerException {
        super(camera);
        this.gameCameraFocusable = gameCameraFocusable;
        this.gameRooms.putAll(gameRooms);
        this.transitionTimer = transitionTimer;
        if (gameCameraFocusable == null || camera == null || gameRooms.isEmpty()) {
            throw new RoomsGameCameraHandlerException(
                    "Fields gameCameraFocusable and gameRooms cannot be null, game rooms cannot be empty, and transition " +
                            "duration must be greater than 0. gameCameraFocusable: [" + gameCameraFocusable + "], camera: " +
    ...
    /**
     * Returns if the {@link GameCameraFocusable} is not contained in any of the game rooms.
     *
     * @return if gameCameraFocusable is not contained in any of the game rooms
     */
    public boolean isFocusableBoundingBoxInAnyGameRoom() {
        return gameRooms.keySet().stream().anyMatch(
                gameRoom -> gameRoom.overlaps(gameCameraFocusable.getBoundingBox()));
    }

    private Rectangle nextGameRoom() {
        return gameRooms.keySet().stream().filter(
                gameRoom -> gameRoom.contains(gameCameraFocusable.getFocus())).findFirst().orElse(null);
    }
    ...
    public void update(float delta)
        ...
        if (transitionState == null) {
            /*
             ...
             case 2: if current game room contains gameCameraFocusable, then set camera position to getFocus and correct bounds
             if applicable

             case 3: if current game room is not null and doesn't contain gameCameraFocusable, then set next game room,
             and if next game room is a neighbour, then init transition process, otherwise jump directly to
             gameCameraFocusable on next update cycle
             */
            if (currentGameRoom == null) {
                currentGameRoom = nextGameRoom();
            } else if (currentGameRoom.contains(gameCameraFocusable.getFocus())) {
                Vector2 focus = gameCameraFocusable.getFocus();
                ...
                transitionDirection = UtilMethods.getOverlapPushDirection(
                        gameCameraFocusable.getBoundingBox(), currentGameRoom, overlap);
